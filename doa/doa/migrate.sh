...
python ${WORKDIR}/analyzers/analyze_sqls.py -n "${APP_NAME}" -i "${REPO_DIR}" -o "${OUTDIR}"

echo 
cyan "analyzing app start-up scripts..."
python ${WORKDIR}/analyzers/analyze_initdb.py -n "${APP_NAME}" -i "${REPO_DIR}" --init-file "${init_file}" -o "${OUTDIR}"
