...
# the first argument is repository URL by default.
# if "-f" is specified, directory of SQL files.
shift $((OPTIND-1))
if (($#==0)); then
    usage
    exit 1
...
echo "running verifier container..."
# set -x
...
docker run --rm -d \
    ...
    -v "${WORKDIR}/doa:/app/doa" \
    -v "${REPO_URL}:/app/sql" \
    -v "/tmp/verify:/tmp/verify:cached" \
    postgres:13.5-alpine > /dev/null
sleep 5
mkdir -p /tmp/verify
docker exec -it doa-verify /app/doa/verify.sh /tmp/verify/stat.txt
docker stop doa-verify > /dev/null
