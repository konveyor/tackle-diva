...
#   docker run -it --rm -v $(readlink -f <outdir>):/out diva-doa:latest \
...
if  [[ -n ${files+x} ]]; then
    ...
    docker run -it --rm \
        ...
        -v "${files_mount}" \
        ${IMAGE} \
        bash -l /work/translate.sh -o /out ${init_file_opt} ${lang+"-l"} ${lang} ${files:+"-f"} ${REPO_URL} # arguments to migrate.sh in container
else
    # set -x
    docker run -it --rm \
        -u vscode \
