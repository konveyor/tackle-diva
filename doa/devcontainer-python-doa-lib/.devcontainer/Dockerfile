...
# DOA uses (tr and) nkf in pre-processing and psql in syntax verification.
RUN apt-get update && export DEBIAN_FRONTEND=noninteractive \
    && apt-get -y install --no-install-recommends nkf bash-completion postgresql
...
RUN su vscode basl -lc "poetry completions bash > /tmp/poetry.bash-completion"  
RUN mv /tmp/poetry.bash-completion /etc/bash_completion.d/poetry.bash-completion
