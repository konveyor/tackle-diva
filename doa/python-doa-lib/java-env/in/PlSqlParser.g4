...
parallel_clause
    : NOPARALLEL
    | PARALLEL ( parallel_count=UNSIGNED_INTEGER | '(' DEGREE parallel_count=UNSIGNED_INTEGER ')' )?
    ;
...
physical_attributes_clause
    : (PCTFREE pctfree=UNSIGNED_INTEGER
      ...
      | MAXTRANS maxtrans=UNSIGNED_INTEGER  // added
      | storage_clause
      )+
    ;
...
comment_on_column
    : COMMENT ON COLUMN column_name IS quoted_string (';' | '/') // modified
    ;
...
comment_on_table
    : COMMENT ON TABLE tableview_name IS quoted_string (';' | '/') // modified
    ;
...
regular_id
    : REGULAR_ID
    | non_reserved_keywords_pre12c // to be commented out by shinsa82
    | non_reserved_keywords_in_12c // to be commented out by shinsa82
    | A_LETTER
    | AGENT
    | AGGREGATE
    | ANALYZE
