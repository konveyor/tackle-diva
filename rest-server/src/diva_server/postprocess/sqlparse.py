...
from .peg import before, choice
from .peg import match as match0
from .peg import nil, pegcxt, pegop, seq, star, val
...
paren = seq(op('('), star(seq(until('(', ')'), lambda s: paren(s))),
            until('(', ')'), op(')'))
...
def comma_sep(e): return seq(e, star(seq(op(','), e)))
def plus(e): return seq(e, star(e))

# [{ s[-1][None] : s[0] }] if len(s) == 2 and seq(qname, option(op('as')), name)(v) == ('', []) else


def ascxt(e): return match(seq(e, option(seq(option(op('as')), match(name, lambda s, v: [{None: v}])))),
                           lambda s, v:
...
callexp = choice(
    ...
    seq(op('cast'), op('('), lambda s: colexp(s), op(
        'as'), until_ignoring_paren(')'), op(')')),
    seq(func, op('('), callargs, op(')'), option(seq(op('over'), paren))))

caseexp = seq(
    op('case'),
    option(lambda s: colexp(s)),
    plus(seq(op('when'), until_ignoring_paren(
...
def frmcxt(e): return match(e, lambda s, _: [{':from': s}])
...
condexp = seq(choice(
    ...
    seq(colexp1,
        choice(
            ...
            variable)),
    seq(choice(op(':_where_and'), op(':_or'), op(':_and')),
        op('('), comma_sep(lambda s: condexp(s)), op(')')),
...
orderexp = seq(op('order'), op('by'), comma_sep(
...
frmspec = seq(
    ...
    option(orderexp),
    option(variable),
    option(seq(op('fetch'), op('first'), choice(
        literal, variable, nil), op('rows'), op('only'))),
...
selexp = withcxt(unioncxt(seq(match(op('select')), option(op('distinct')), option(seq(
    op('top'), literal)), colsexp, option(op(',')), option(seq(op('into'), colsexp)), op('from'), frmspec)))
updexp = withcxt(seq(match(op('update')), frmcxt(
    seq(ascxt(match(qname)), op('set'), assignsexp)), option(whrexp)))
...
delexp = withcxt(seq(match(op('delete')), option(op('from')),
                 frmcxt(ascxt(match(qname))), option(whrexp)))
